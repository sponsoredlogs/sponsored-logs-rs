...
- **Brand-safe gilding: the gold `[AD]` standard.** The `[AD]` tag now renders in premium 256-color gold (xterm 214) on a live terminal, turning a plain tag into a high-visibility trust signal at peak incident attention. The escapes are zero-width, so banner mastheads stay pixel-aligned byte-for-byte. `Color::Auto` (the default) gilds only when stdout is a TTY and `NO_COLOR` is unset, since the crate emits through your subscriber and must never leak ANSI into a file sink; `Color::Always` and `Color::Never` are deterministic. Also configurable via `SPONSORED_LOGS_COLOR`. Consent is our moat ([#4](https://github.com/sponsoredlogs/sponsored-logs-rs/pull/4))
